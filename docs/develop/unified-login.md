...
所有产品共用 [`LuminaryWorks/identity`](https://github.com/LuminaryWorks/identity)（Logto OIDC）。**Logto 负责 AuthN + 产品准入；各产品用 Casbin 负责资源 AuthZ。** 登录 UI 默认走 **Experience API（Headless）**，按产品品牌自建页面。
...
**原则**：Logto（或企业 IdP）只管身份；产品本地保留 `user_id ↔ sub` 映射与 Casbin 资源 ACL，方便日后业务剥离。
...
产品 SPA (@luminary/auth-react)  →  NestJS (@luminaryworks/auth-core)  →  Casbin（资源）
```
...
产品**不要**直连某一家 IdP 的专有 API；只认 OIDC issuer（推荐经 [Auth Gateway](./auth-gateway)）。
...
JWT 只含 `sub` / 租户 / 平台准入；**业务权限与商业权益不进 Token**。
...
| 模型 | `sub, obj, act` |
| 映射 | IdP `sub` → 本地 `user_id`（首次登录 upsert） |
